...
import { BudgetValidate } from "../../../model/common/import/budget-validate";
...
@Injectable()
export class BudgetForecastHttpService extends GenericHttpService<any, number> {
  constructor(public http: HttpClient) {
    super(http, "", "budgetforecasts");
  }
  ...
  addNewBudget(item: BudgetSave): Observable<any> {
    let url: string = AppConfig.urlPrefix + this.url + "/detail";

    return this.http
      .post(url, JSON.stringify(item), { headers: this.headers }).pipe(
      map((data: Response) => {
        return data;
      }));
  }

  updateAsset(item: BudgetSave): Observable<any> {
    let url: string = AppConfig.urlPrefix + this.url + "/detail";
...
public subType(subTypeId: number, accMonthId: number): Observable<any> {
  ...
  public budgetValidate(reco: Array<BudgetValidate>): Observable<any> {
    // return super.create(reco, 'reco')
    return this.http
      .post(
  ...
  public import(item: BudgetBaseImport): Observable<ImportBudgetResult> {
    return this.http.post(AppConfig.urlPrefix + this.url + '/import', item).pipe(
        map((data: ImportBudgetResult) => {
            return data;
        }));
}
...
public getData(): Observable<any[]> {
  return this.http.get(AppConfig.urlPrefix + this.url + '/tree/', { headers: this.headers }).pipe(
  map((data: any[]) => {
      return data;
  }));
...
updateBudgetForecast(item: BudgetSave): Observable<RequestResult> {
...
updateAssetBudgetForecast(item: BudgetForecastUpdate): Observable<RequestResult> {
  const url: string = AppConfig.urlPrefix + this.url + '/updateAssetBudgetForecast';
  return this.http
    .post(url, item, { headers: this.headers }).pipe(
    map((data: RequestResult) => {
      return data;
...
correctionBudgetForecast(item: BudgetCorrectionSave): Observable<RequestResult> {
  const url: string = AppConfig.urlPrefix + this.url + '/correction';
  return this.http
    .post(url, item, { headers: this.headers }).pipe(
    map((data: RequestResult) => {
      return data;
...
transferBudgetForecast(item: BudgetTransferSave): Observable<BudgetForecastCorrrectionResult> {
  const url: string = AppConfig.urlPrefix + this.url + '/transfer';
  return this.http
    .post(url, item, { headers: this.headers }).pipe(
    map((data: BudgetForecastCorrrectionResult) => {
      return data;
    }));
}

correctionValidateBudgetForecast(item: BudgetCorrectionValidation): Observable<RequestResult> {
  const url: string = AppConfig.urlPrefix + this.url + '/validation';
  return this.http
    .post(url, item, { headers: this.headers }).pipe(
    map((data: RequestResult) => {
      return data;
