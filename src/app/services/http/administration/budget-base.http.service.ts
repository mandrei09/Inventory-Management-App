...
@Injectable()
export class BudgetBaseHttpService extends GenericHttpService<any, number> {
  constructor(public http: HttpClient) {
    super(http, '', 'budgetbases');
  }
  ...
  updateBudget(item: BudgetSave): Observable<any> {
  ...
  updateRequestBudget(item: BudgetAddSave): Observable<RequestResult> {
    const url: string = AppConfig.urlPrefix + this.url + '/addRequest';
    return this.http
      .post(url, item, { headers: this.headers }).pipe(
...
public subType(subTypeId: number, accMonthId: number): Observable<any> {
  ...
  public import(item: BudgetBaseImport): Observable<any> {
