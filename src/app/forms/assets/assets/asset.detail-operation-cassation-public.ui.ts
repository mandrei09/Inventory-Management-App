...
import { UploadPublicStornoModalComponent } from '../../common/upload-public-storno-modal.component';

@Component({
    selector: 'app-asset-detail-operation-cassation-public-ui',
    templateUrl: 'asset.detail-operation-cassation-public.ui.html',
    styleUrls: ['asset.detail-operation-cassation-public.ui.scss'],
...
export class AssetDetailUIOperationCassationPublicComponent implements AfterViewInit, OnInit  {
    ...
    public entityTypeCode: string = 'STORNO_PUBLIC';
    ...
    public get allowSaving(): boolean {
            ...
            this.uom != null && this.entityFileList.items.length > 0;
    }
    ...
    public refreshEntityFiles() {
        ...
        params.push(new Param('entityTypeCode', 'STORNO_PUBLIC'));
    ...
    public saveAsset() {
        ...
        saveRetireAsset.completeRetire = true;
        saveRetireAsset.priorYearAcquisitions = false;
        saveRetireAsset.currentYearAcquisitions = true;
        saveRetireAsset.amount = 0;
        saveRetireAsset.percent = 100;
        saveRetireAsset.quantity = 0;
        saveRetireAsset.uomId = this.uom.id;
        saveRetireAsset.assetId = this.asset.id;

        if (saveRetireAsset != null) {
            this.assetHttpService.publicRetireAsset(saveRetireAsset)
...
public deleteOperation(){
    ...
    onAssetFileUpload() {
      const dialogRef = this.dialog.open(UploadPublicStornoModalComponent, {
        panelClass: 'centered-middle-modal', height: '80%', maxHeight: '80%', disableClose: true, width: '600px', position: { bottom: '15%', top: 'auto'},
        data: { uploadType: 'STORNO_PUBLIC', uploadFolder: 'STORNO_PUBLIC', assetId: this.assetId }
      });
