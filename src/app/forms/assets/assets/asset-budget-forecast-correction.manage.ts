...
@Component({
    selector: 'app-asset-budget-forecast-correction-manage',
    templateUrl: 'asset-budget-forecast-correction.manage.html',
    styleUrls: ['asset-budget-forecast-correction.manage.scss'],
    providers: [
        AccMonthHttpService,
...
export class AssetBudgetForecastCorrectionManageComponent implements OnInit, AfterViewInit { // extends GenericManage<AssetInvDetail> {

    public _budgetForecasts: BudgetForecast = null;
    public get budgetForecasts(): BudgetForecast { return this._budgetForecasts; }
    public set budgetForecasts(value: BudgetForecast) {
      this._budgetForecasts = value;

      this.selectedBudgetForecast = value;
      this.checkForRefresh();
    }

    public get budgetForecastParams(): Array<Param> {
      const params: Array<Param> = [];
      const budgetFilter: BudgetFilter = new BudgetFilter();
      this.projectId = this.selectedAssets.length > 0 && this.selectedAssets[0].adm.project != null  ? this.selectedAssets[0].adm.project.id : null;
      budgetFilter.projectId = this.projectId;
...
public selectedBudgetForecast: BudgetForecast = null;
    ...
    constructor(public route: ActivatedRoute,
            ...
            this.router.events.subscribe((evt) => {
                if (evt instanceof NavigationEnd) {
                    if (evt.urlAfterRedirects === '/budget/correction') {
    ...
    public cloneAsset() {
        const selectedAssetId = this.selectedAssets.length > 0 ? this.selectedAssets[0].id : 0;
        if (selectedAssetId > 0) {
            this.router.navigate(['/assetclone', selectedAssetId]);
        }
    }
   ...
   /* begin room */
    ...
    public deleteAsset() {
        ...
        public blockAsset() {
            if (confirm('Esti sigur ca vrei sa blochezi acest obiect?')) {
    ...
    public getFilters(): Array<Param> {
        ...
        assetFilter.export = 'stockhistory';
        assetFilter.historyMeniu = true;
        params.push(new Param('historyMeniu', 'true'));
...
public selectBudgetForecast() {
  ...
  this.budgetChangeForecastList.refresh(params);
}
...
public updateBudgetBase() {
  ...
  this.budgetForecastHttpService.updateAssetBudgetForecast(data).subscribe((res) => {
      ...
      if (res.success) {
          this.notifyService.showSuccess(res.message, 'Actualizare modificari', 0, false, 0);
          this.assetList.refresh(null);
      } else {
          this.notifyService.showError(res.message, 'Eroare salvare date!', 0, false, 0);
          this.assetList.refresh(null);
      }
  }, (error) => {
      this.notifyService.showError('', 'Eroare server!', 0, false, 0);
