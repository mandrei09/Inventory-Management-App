...
@Component({
    selector: 'app-asset-detail-validate-inv-plus-ui',
    templateUrl: 'asset.detail-validate-inv-plus.ui.html',
    styleUrls: ['asset.detail-validate-inv-plus.ui.scss'],
...
export class AssetDetailUIValidateInvPlusComponent implements AfterViewInit  {
    ...
    public asset: UpdateAssetInvPlus = new UpdateAssetInvPlus();
    public addAsset: AddAssetInvPlus = new AddAssetInvPlus();
    ...
    public updateAsset() {
        ...
        if (this.asset.id > 0) {
            this.assetHttpService.updateAssetInvPlus(this.asset)
           .subscribe((result: CreateAssetSAPResult) => {
               if (result.success) {
                   this.notificationService.showSuccess(result.errorMessage, 'Actualizare informatii', 0, false, 0);
    ...
    public createAsset() {
        ...
        this.isSaved = false;
        this.addAsset.assetId = this.asset.id;
        // this.addAsset.companyId = this.order != null && this.order.offer != null && this.order.offer.company != null ? this.order.offer.company.id : 5;
        // this.addAsset.costCenterId = this.costCenter != null ? this.costCenter.id : null;
        // this.addAsset.employeeId = this.employee != null ? this.employee.id : null;
        // this.addAsset.partnerId = this.partner != null ? this.partner.id : null;

        // this.addAsset.materialId = this.material != null ? this.material.id : null;

        // this.addAsset.name = this.dictionaryItem != null ? this.dictionaryItem.name : this.asset.name;
        // this.addAsset.orderId = this.order != null ? this.order.id : null;
        // this.addAsset.documentTypeId = this.documentType != null ? this.documentType.id : null;
        // this.addAsset.dictionaryItemId = this.dictionaryItem != null ? this.dictionaryItem.id : null;
        // this.addAsset.name2 = this.asset.name;
        // this.addAsset.subCategoryId = this.subCategory != null ? this.subCategory.id : null;
        // this.addAsset.orderMaterialId = this.orderMaterialList.selectedItems.length > 0 ? this.orderMaterialList.selectedItems[0].id : 0;

        this.assetHttpService.addAssetInvPlus(this.addAsset)
        .subscribe((result: CreateAssetSAPResult) => {
            if (result.success) {
                this.notificationService.showSuccess('Operatia a fost finalizata cu success!', 'Adaugare asset', 0, false, 0);
                this.router.navigate(['/asset']);
            } else if (!result.success) {
                this.notificationService.showError('Motiv: ' + result.errorMessage + '!', 'Eroare salvare date', 0, false, 0);
            }
