...
import { AssetOutSAP, SaveRetireAsset, SaveStornoAsset } from '../../../model/api/assets/asset-out';
...
@Component({
    selector: 'app-asset-detail-acquisition-operation-storno-ui',
    templateUrl: 'asset.detail-acquisition-operation-storno.ui.html',
    styleUrls: ['asset.detail-acquisition-operation-storno.ui.scss'],
...
export class AssetDetailUIAcquisitionOperationStornoComponent implements AfterViewInit, OnInit  {
    ...
    public updateAssetDetails(asset: any) {
        ...
        this.refDocNo = asset.lastAcquisition.length > 0 ?  asset.lastAcquisition[0].belnr : '';
    ...
    public saveAsset() {
        this.isSaved = false;
        const save = new SaveAssetAcquisitionStorno();
        save.assetIds = new Array<number>();
        save.refDocNo = this.refDocNo;

        this.acquisitionList.items.forEach(element => {
            save.assetIds.push(element.id);
        });

        if (save != null) {
            this.assetHttpService.newAssetAcquisitionStorno(save)
