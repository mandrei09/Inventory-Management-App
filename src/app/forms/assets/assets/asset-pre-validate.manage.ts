...
import {AssetPreValidateDialogComponent} from './asset-pre-validate-dialog/asset-pre-validate-dialog.component';
import { UpdateAssetSAPResult } from '../../../model/api/result/update-asset-SAP-result';
import { AssetPreValidateAddEditDialog } from './asset-pre-validate.add-edit.dialog';
import { Asset } from '../../../model/api/assets/asset';
@Component({
    selector: 'app-asset-pre-validate-manage-component',
    templateUrl: 'asset-pre-validate.manage.html',
    styleUrls: ['asset-pre-validate.manage.scss'],
    providers: [
        AccMonthHttpService,
...
export class AssetPreValidateManageComponent implements OnInit, AfterViewInit { // extends GenericManage<AssetInvDetail> {
...
//   };
    ...
    constructor(public route: ActivatedRoute,
            ...
            this.router.events.subscribe((evt) => {
                if (evt instanceof NavigationEnd) {
                    if (evt.urlAfterRedirects === '/asset/pre/validate') {
    ...
    public onAddEditItem(item: Asset | null = null) {
        let dialogRef = this.dialog.open(AssetPreValidateAddEditDialog, {
            panelClass: 'centered-middle-modal', width: '80%', maxWidth: '80%', maxHeight: '50%', height: 'auto', position: {top: '30em'},
          data: { item: item }
        });
  
        dialogRef.afterClosed().subscribe((item: Asset) => {
          if (item !== null) this.refreshAssets();
        });
      }
  
      public onItemEdit(item: Asset) {
    ...
    public preValidateAsset() {
        const selectedAssetId = this.selectedAssets.length > 0 ? this.selectedAssets[0].id : 0;
        if (selectedAssetId > 0) {
            // if (this.selectedAssets[0].isLocked) {
            //     this.notifyService.showWarning('Numarul de inventar ' + this.selectedAssets[0].invNo + ' este blocat!', '', 0, false, 0);
            //     return;
            // }
            // this.router.navigate(['/assetprevalidate', selectedAssetId]);
            this.onPreValidateAssetDialog(selectedAssetId);
        }
    }

    public onPreValidateAssetDialog(selectedAssetId) {
      let dialogRef = this.dialog.open(AssetPreValidateDialogComponent, {
        panelClass: 'large-modal', disableClose: true, width: '100%', maxWidth: '100vw', maxHeight: '100vh', height: 'calc(100vh - 55px)', position: { bottom: '0' },
        data: {uploadType: "PRE_RECEPTION", assetId: selectedAssetId }
      });

      dialogRef.afterClosed().subscribe((item: UpdateAssetSAPResult) => {
...
public exportPrereception() {
    ...
    params = this.getFilters();
    this.assetHttpService
        .exportPrereception(params)
        .subscribe((blob) => {
    fileSaveAs(blob.body, 'Prereception.xlsx');
