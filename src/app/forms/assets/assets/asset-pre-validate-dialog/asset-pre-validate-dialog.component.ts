...
@Component({
  ...
  templateUrl: "./asset-pre-validate-dialog.component.html",
  styleUrls: ["./asset-pre-validate-dialog.component.scss"],
})
export class AssetPreValidateDialogComponent implements AfterViewInit {
  ...
  @ViewChild("rateNewItemListModal")
  public rateNewItemListModal: ModalDirective;
  ...
  @ViewChild("costCenterDetail")
  public costCenterDetail: CostCenterDetailComponent;
  @ViewChild("costCenterList") public costCenterList: CostCenterListComponent;
  @ViewChild("costCenterDetailModal")
  public costCenterDetailModal: ModalDirective;
  ...
  @ViewChild("documentTypeDropDownList")
  public documentTypeDropDownList: DocumentTypeDropDownListComponent;
  @ViewChild("fileInput") fileInput;

  @ViewChild("rejectModal") public rejectModal: ModalDirective;

@ViewChild("acquisitionList")
  public acquisitionList: AcquisitionValidateReadOnlyOfferListComponent;
  // @ViewChild("acquisitionSAPList")
  // public acquisitionSAPList: AquisitionAssetSAPListComponent;
  ...
  public get allowSaving(): boolean {
      return (
        this.asset != null && this.asset.name != null &&
        this.tax != null && this.asset.headerText != null &&
        this.asset.headerText.trim().length > 0 &&
        this.asset.docNo1 != null &&
        this.asset.docNo1.trim().length > 0 &&
        this.asset.quantity > 0 && this.tax != null && this.invoiceDate != null &&
        this.asset.rateValue != null && this.asset.assetValueDate != null && this.assetCategory != null &&
        this.partner != null && this.asset.subNo != null &&
        this.material.name != null && this.material.name !== '' && this.asset.valueInvRon > 0 &&
        this.expAccount != null && this.uom != null && this.subCategory != null && this.costCenter != null &&
  ...
  constructor(
    ...
    private dialogRef: MatDialogRef<AssetPreValidateDialogComponent>,
  ...
  public updateDetails(asset: any) {
   ...
   // this.subCategory = asset.subCategory;
    ...
    if (asset.invoiceDate === "0001-01-01T00:00:00") {
      this.asset.invoiceDate = null; 
    }
  ...
  public selectDictionaryItem() {
    ...
    params.push(
      new Param(
        "assetCategoryIds",
        AppUtils.getIdsList<AssetCategory, number>(this.selectedAssetCategories)
      )
    );
  ...
  async searchPartnerByCUI() {
    ...
    const request = new PartnerRequest(
      this.partner.registryNumber,
      this.getNowDate(),
      this.partner.id
    );
    // request.push(new PartnerRequest(this.partnerSearchCUI, this.getNowDate()));
    this.partnerHttpService.requestByCUI(request).subscribe(
      async (res: boolean) => {
  ...
  async getPartnerByCorrelationId(correlationId: string) {
    this.partnerHttpService.requestByCorrelationId(correlationId).subscribe(
      async (response: PartnerCorrelationResponse) => {
  ...
  public deleteOrderitem() {
    this.assetHttpService
      .deleteOrderItem(this.selectedAcquisitionAsset.id)
      .subscribe((result: AssetResult) => {
        if (result.success) {
          this.notificationService.showHTMLMessage(
            "<h6>" + result.errorMessage + "!!</h6>",
            "Stergere produs comanda"
          );
  ...
  changeInvoiceDate(event: MatDatepickerInputEvent<Date>) {
    this.invoiceDate = event.value.toString();
    this.asset.invoiceDate = event.value;
    this.rate = null;
  }

  changeAssetValueDate(event: MatDatepickerInputEvent<Date>) {
    // console.log(event.target.value);
    this.assetValueDate = event.value.toString();
    // this.rate = null;
  }

  updateAssetParent(){
    this.asset.AssetParentId =  this.assetEntity != null ? this.assetEntity.id : null;
    
    if (this.assetEntity.id > 0) {
      this.assetHttpService.updateParentNumber(this.asset).subscribe((res: UpdateAssetSAPResult) => {
        if (res.success) {
          this.notificationService.showInfo( res.errorMessage, "Actualizare date", 5000, 0, 0);
        }
      });
    }
  }
  ...
  public updateAsset() {
    ...
    this.asset.expAccountId =
      this.expAccount != null ? this.expAccount.id : null;
    this.asset.assetCategoryId =
      this.assetCategory != null ? this.assetCategory.id : null;
    this.asset.partnerId = this.partner != null ? this.partner.id : null;
    this.asset.articleId = this.article != null ? this.article.id : null;
    this.asset.costCenterId =
      this.costCenter != null ? this.costCenter.id : null;
    this.asset.divisionId = this.division != null ? this.division.id : null;
    this.asset.departmentId =
      this.department != null ? this.department.id : null;
    this.asset.roomId = this.room != null ? this.room.id : null;
    this.asset.budgetManagerId =
      this.budgetManager != null ? this.budgetManager.id : null;
    this.asset.assetNatureId =
      this.assetNature != null ? this.assetNature.id : null;
    ...
    this.asset.dictionaryItemId =
      this.dictionaryItem != null ? this.dictionaryItem.id : null;
    ...
    if(this.asset.rateValue == null)
    {
      this.asset.rateValue = 1
    }

    if (this.invoiceDate != "")
      this.asset.invoiceDate = new Date(this.invoiceDate);
    else this.asset.invoiceDate = new Date();

    if (this.assetValueDate != "")
      this.asset.assetValueDate = new Date(this.assetValueDate);
    else this.asset.assetValueDate = new Date();

    this.asset.removalDate = this.asset.removalDate;

    this.asset.validated = true;

    this.asset.subCategoryId =
      this.subCategory != null ? this.subCategory.id : null;

    if (this.asset.id > 0) {
      this.assetHttpService
        .updatePreAcquisition(this.asset)
  ...
  public logg(){
    ...
    console.log(this.acquisitionList.selectedItem)
  }
  ...
  public approveAsset() {
    ...
    if (this.asset.id > 0) {
      this.assetHttpService
        .approvePreReception(this.asset.id)
        .subscribe((res: UpdateAssetSAPResult) => {
          if (res.success) {
            this.notificationService.showInfo(
              res.errorMessage,
              "Aprobare pre -receptie",
              5000,
              0,
              0
            );
  ...
  public saveAsset() {
    ...
    if (this.asset.id > 0) {
      this.assetHttpService.validateAsset(this.asset).subscribe(() => {
        this.assetHttpService
          .getDetailById(this.asset.id)
          .subscribe((asset: any) => {
            if (asset != null) {
  ...
  public validateAcqizitionAsset() {
    ...
    if (this.asset.id > 0) {
      if (this.asset.storno) {
        this.assetHttpService
          .validateStornoAcquizitionAsset(this.asset)
          .subscribe((result: CreateAssetSAPResult) => {
            if (result.success) {
              this.notificationService.showSuccess(
                result.errorMessage,
              ...
              const assetFilter: AssetReceptionFilter =
                new AssetReceptionFilter();
        ...
        this.assetHttpService
          .validateAcquizitionAsset(this.asset)
          .subscribe((result: CreateAssetSAPResult) => {
  ...
  public onRejectAsset() {
    ...
    this.dialogService
      ...
      .subscribe((confirmedReason: string) => {
        if (confirmedReason) {
          this.reason = confirmedReason;
  ...
  public deleteOperation() {
    if (confirm("Esti sigur ca vrei sa respingi assetul selectat?")) {
      this.assetHttpService
        .rejectAsset(this.assetId, this.reason)
        .subscribe((res: RejectResult) => {
          if (res.success) {
            this.notificationService.showInfo(
              res.message,
              "Refuz validare",
  ...
  public checkUniqueDocument(event: any) {
    ...
    if (
      ...
      this.asset.docNo1 !== "" &&
      this.asset.docNo1 != null &&
      this.asset.docNo1 !== undefined &&
      this.asset.docNo1.trim().length > 3
    ) {
      this.checkUnique = false;
      this.assetHttpService
        .checkUniqueDocumentAndPartner(
          this.asset?.docNo1,
          this.partner?.id,
      ...
      this.notificationService.showWarning(
        "Lungimea factura prea scurta pentru verificare!",
        "Verificare unicitate factura",
        0,
        false,
        0
  ...
  public onAcquisitionListSelectionChanged(items: Array<any>) {
    ...
    if (
      this.selectedAcquisitionAsset != null &&
      this.selectedAcquisitionAsset.id > 0
    ) {
      this.assetHttpService
        .getDetailById(this.selectedAcquisitionAsset.id)
        .subscribe((asset: any) => {
          // this.asset = asset;
          this.initialInvNo = asset.invNo.toString().trim();
          this.updateDetails(asset);
  ...
  public setSelectedMaterial() {
    ...
    materialsIds.offerId =
      this.selectedAcquisitionAsset != null &&
      this.selectedAcquisitionAsset.order != null &&
      this.selectedAcquisitionAsset.order.offer != null
        ? this.selectedAcquisitionAsset.order.offer.id
        : 0;
    ...
    this.offerMaterialHttpService
      .addMaterialByOfferFromAsset(materialsIds)
      .subscribe(
        (res) => {
  ...
  onAssetFileUpload() {
    const dialogRef = this.dialog.open(UploadPreReceptionModalComponent, {
      ...
      data: { uploadFolder: 'PRE_RECEPTION', assetId : this.assetId}
    });
