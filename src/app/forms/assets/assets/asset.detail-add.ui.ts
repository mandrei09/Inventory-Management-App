...
import { OrderMaterialListComponent } from '../../administrations/order-materials/order-material.list';
...
@Component({
    selector: 'app-asset-detail-add-ui',
    templateUrl: 'asset.detail-add.ui.html',
    styleUrls: ['asset.detail-add.ui.scss'],
...
export class AssetDetailUIAddComponent implements AfterViewInit  {
    ...
    public saveAsset() {
        ...
        this.entityFileHttpService.addAssetFiles(this.filesToUpload, guid, 'NEWASSET').subscribe( (res: EntityFileResult) => {
        ...
        // this.asset.validated = true;
            ...
            .subscribe((result: CreateAssetSAPResult) => {
                ...
                if (result.success) {
                    this.notificationService.showSuccess(res.message, 'Adaugare asset', 0, false, 0);
                    this.router.navigate(['/asset']);
        ...
        .subscribe((result: CreateAssetSAPResult) => {
            if (result.success) {
                this.notificationService.showSuccess(result.errorMessage, 'Adaugare asset', 0, false, 0);
                this.router.navigate(['/asset/reception']);
            } else if (!result.success) {
                this.notificationService.showError('Motiv: ' + result.errorMessage + '!', 'Eroare salvare date', 0, false, 0);
                this.isSaved = true;
