...
import {AssetAddDialogComponent} from './asset-add-dialog/asset-add-dialog.component';
import {MatDialog} from '@angular/material/dialog';
import { CreateAssetSAPResult } from '../../../model/api/result/create-asset-SAP-result';
import {AssetReceptionDetailsDialog} from './asset-reception-details/asset-reception-details.dialog';
import { RejectResult } from "../../../model/api/result/reject-result";
import { DialogService } from '../../../services/dialog.service';
@Component({
    selector: 'app-asset-reception-manage',
    templateUrl: 'asset-reception.manage.html',
    styleUrls: ['asset-reception.manage.scss'],
    providers: [
        AccMonthHttpService,
...
export class AssetReceptionManageComponent implements OnInit, AfterViewInit { // extends GenericManage<AssetInvDetail> {
    ...
    public onAddItem() {
      let dialogRef = this.dialog.open(AssetAddDialogComponent, {
        ...
        data: {order: null, isPreReception: false}
      });
    ...
    public editAsset() {
        if(this.selectedAssets.length > 0)
        {
            ...
            if (asset.id > 0) 
            {
                const sapCodePrefix = 'RO10'
                if(asset.invNo.startsWith(sapCodePrefix))
                {
                    this.notificationService.showWarning('Nu s-a realizat actualizarea cu SAP pentru produsul selectat!', '', 0, false, 0)
                }
                else
                {
                    this.onReceptionDialog(asset.id);
                }
            }
        }
    }
    ...
    public rejectAsset() {
        ...
        .subscribe((confirmedReason: string) => {
            ...
            this.onRejectConfirmationApproved(selectedAssetId);
            }
        });
        }
    }

    public onRejectConfirmationApproved(assetId : number) {
        switch (this.operationType) {
          case OperationType.RejectAsset:
            this.deleteOperation(assetId);
            break;
          default:
            break;
        }
    ...
    public onReceptionDialog(selectedAssetId) {
      let dialogRef = this.dialog.open(AssetReceptionDetailsDialog, {
        panelClass: 'large-modal', disableClose: true, width: '100%', maxWidth: '100vw', maxHeight: '100vh', height: 'calc(100vh - 55px)', position: { bottom: '0' },
        ...
        if (confirm('Esti sigur ca vrei sa stergi acest obiect?')){
            this.assetHttpService.deleteAsset(this.selectedAssets[0].id).subscribe((res) => {
                if (res) {
                    ...
                    this.notificationService.showSuccess('Datele au fost actualizate cu succes!', '', 0, false, 0);
                    // this.toastr.success('Date au fost actualizate cu success');
                   // this.checkForRefresh()
                    this.clearSelection();
                    this.refreshAssets();
                } else {
                    this.notificationService.showError('Eroare salvare!', '', 0, false, 0);
                }
            }, (error) => {
    ...
    public markPrintData () {
        ...
        .subscribe((res) => {
            if (res.statusCode === 200){
                ...
                this.notificationService.showError('Datele nu au fost actualizate cu succes!', '', 0, false, 0);
            }
    ...
    public markAllPrintData () {
        ...
        .subscribe((res) => {
            if (res){
                ...
                this.notificationService.showSuccess('Eroare. Datele nu au fost actualizate!', '', 0, false, 0);
            }
