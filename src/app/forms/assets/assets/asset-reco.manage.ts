...
import { AssetRecoListComponent } from './asset-reco.list';
import { AssetRecoAllListComponent } from './asset-reco-all.list';
import { AssetRecoHttpService } from '../../../services/http/assets/asset.reco.http.service';
import { AssetRecoAllHttpService } from '../../../services/http/assets/asset-reco-all.http.service';
import { AssetRecoSave } from '../../../model/api/assets/asset-reco-save';
// import { saveAs as fileSaveAs } from 'file-saver-es';
// import { ToastsManager } from 'ng2-toastr';

@Component({
    selector: 'asset-reco-manage',
    templateUrl: 'asset-reco.manage.html',
    styleUrls: ['asset-reco.manage.scss'],
    providers: [
        AccMonthHttpService,
...
export class AssetRecoManageComponent { // extends GenericManage<AssetInvDetail> {


    @ViewChild('importDataModal') public importDataModal: ModalDirective;

    @ViewChild('assetRecoList') public assetRecoList: AssetRecoListComponent;
    @ViewChild('assetRecoAllList') public assetRecoAllList: AssetRecoAllListComponent;
    ...
    public filterDoc: string;
    public filterAllDoc: string;
    public get allowReconciliation(): boolean { return ((this.selectedAssets != null && this.selectedAssets.length === 1) && (this.selectedAssetAlls != null && this.selectedAssetAlls.length === 1)); }
    ...
    public selectedAssetAlls: Array<Uom> = new Array<Uom>();
...
//   };
    ...
    constructor(public route: ActivatedRoute,
                ...
                public assetRecoHttpService: AssetRecoHttpService,
                public assetRecoAllHttpService: AssetRecoAllHttpService,
            ...
            //  this.toastr.setRootViewContainerRef(vcr);
             this.router.events.subscribe((evt) => {
                if (evt instanceof NavigationEnd) {
                    if (evt.urlAfterRedirects === '/assetrecoinvdetails') {
    ...
    public onReconcile() {
        ...
        // if (this.selectedAssets[0].isReconcile || this.selectedAssetAlls[0].asset != null) {
        //     alert('Va rugam selectati active care nu au fost reconciliate');
        //     return;
        // }

        this.assetInvNos = new Array<string>();
        this.assetInvNos.push(this.selectedAssets[0].invNo + '-->'  + this.selectedAssetAlls[0].code);
    ...
    public reconcile() {
        ...
        assetRecoSave.assetId = this.selectedAssets[0].id;
        // assetRecoSave.UomId = this.selectedAssetAlls[0].id;
        this.assetRecoHttpService.saveReco(assetRecoSave).subscribe((res) => {
    ...
    public clearSelection() {
        ...
        this.assetRecoList.selectedItems = this.selectedAssets;
    ...
    public clearAllSelection() {
        this.selectedAssetAlls = new Array<Uom>();
        this.assetRecoAllList.selectedItems = this.selectedAssetAlls;
        // this.selectedAssetInvDetails = new Array<AssetInvDetail>();
        // this.assetList.selectedItems = this.selectedAssetInvDetails;
        // this.selectedAssets = new Array<AssetSimpleDetail>();
    }
    ...
    public saveNewAsset (assets: Array<AssetSimpleDetail>) {
        ...
        // this.assetRecoHttpService.updateAssetName(assetUpdates).subscribe( (res) => {
        //     if (res === 200) {
...
//   });
        if (confirm('Esti sigur ca vrei sa stergi acest obiect?')){
            this.assetRecoHttpService.deleteAsset(this.selectedAssets[0].id).subscribe((res) => {}, (error) => {
    ...
    public refreshAssetAll() {
       ...
       // if (this.depView) this.requestAssetDepDetailRefreshEvent.emit(params);
        // if (this.invView) this.requestAssetInvDetailRefreshEvent.emit(params);
        this.assetRecoAllList.refresh(params);
    }
    ...
    public getFilters(): Array<Param> {
        ...
        // assetFilter.filterAllDoc = this.filterAllDoc;
        ...
        // assetFilter.isPrinted = ((this.isPrinted === '-') ? null : (this.isPrinted === 'YES' ? 'true' : 'false'));
    ...
    public doImportV1() {
        if (this.importIndex < this.importLinesV1.length) {
          //  console.log(JSON.stringify(this.importLinesV1));
            this.assetRecoHttpService.uploadV1(this.importLinesV1[this.importIndex]).subscribe((data) => {
    ...
    public doImportV2() {
        if (this.importIndex < this.importLinesV2.length) {
            this.assetRecoHttpService.uploadV2(this.importLinesV2[this.importIndex]).subscribe((data) => {
                this.importIndex = this.importIndex + 1;
                this.doImportV2();
            });
        }
    ...
    public showPifList(): void {
        let url: string = '';
        url = `${AppConfig.reportingServer}Report.aspx/?report=pifReport&assetId=${this.assetRecoList.selectedItem.id}`;
        window.open(url);
      }

      public showPvList(): void {
        let url: string = '';
        url = `${AppConfig.reportingServer}Report.aspx/?report=pvReport&assetId=${this.assetRecoList.selectedItem.id}`;
        window.open(url);
      }
    ...
    public importDataV2() {
      ...
      public exportToExcel() {
        ...
        this.assetRecoHttpService.get(1, 1000000, 'invNo', 'asc', params, null).subscribe(
...
//         .export(params)
//         .subscribe((blob) => {
//     fileSaveAs(blob, 'Export Active.xlsx');
// });
}
    ...
    //     // });
    //     this.assetRecoAllHttpService.printLocalLabel(label).subscribe( (res: PrintLabel) => {
    //         if (res) {
    ...
    public printLabel (){
        ...
        // });
        // this.assetRecoHttpService.printLocalLabel(label).subscribe( (res: PrintLabel) => {
        //     if (res) {
        //         this.markPrintData();
        //     }
        // }, (error) => {
