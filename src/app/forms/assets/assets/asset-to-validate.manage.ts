...
import {AssetValidateDialogComponent} from './asset-validate-dialog/asset-validate-dialog.component';
...
@Component({
    selector: 'app-asset-to-validate-manage',
    templateUrl: 'asset-to-validate.manage.html',
    styleUrls: ['asset-to-validate.manage.scss'],
    providers: [
        AccMonthHttpService,
...
export class AssetToValidateManageComponent implements OnInit, AfterViewInit { // extends GenericManage<AssetInvDetail> {
...
//   };
    ...
    constructor(public route: ActivatedRoute,
            ...
            this.router.events.subscribe((evt) => {
                if (evt instanceof NavigationEnd) {
                    if (evt.urlAfterRedirects === '/asset/validate') {
    ...
    public openNewOperationDialog() {
      const dialogRef = this.dialog.open(NewOperationDialogComponent, {
        ...
        // data: { selectedItems: this.itemList.selectedItems }
      });
    ...
    public validateAsset() {
        ...
        if (selectedAssetId > 0) {
            if (this.selectedAssets[0].isLocked) {
                this.notificationService.showWarning('Numarul de inventar ' + this.selectedAssets[0].invNo + ' este blocat!', '', 0, false, 0);
                return;
            }
            // this.router.navigate(['/assetvalidate', selectedAssetId]);
            this.onValidateAsset(selectedAssetId);
        }
    }

    public onValidateAsset(selectedAssetId) {
      let dialogRef = this.dialog.open(AssetValidateDialogComponent, {
        panelClass: 'large-modal', disableClose: true, width: '100%', maxWidth: '100vw', maxHeight: '100vh', height: 'calc(100vh - 55px)', position: { bottom: '0' },
    ...
    public setSelectedCostCenters(value: any) {
        this.selectedCostCenters = value;
