...
import { AssetOutSAP, SaveRetireAsset, SaveStornoAsset, SaveStornoAssetMFX } from '../../../model/api/assets/asset-out';
import { CreateAssetSAPResult } from '../../../model/api/result/create-asset-SAP-result';

@Component({
    selector: 'app-asset-detail-operation-storno-mfx-ui',
    templateUrl: 'asset.detail-operation-storno-mfx.ui.html',
    styleUrls: ['asset.detail-operation-storno-mfx.ui.scss'],
...
export class AssetDetailUIOperationStornoMFXComponent implements AfterViewInit, OnInit  {
    ...
    public saveAsset() {
        this.isSaved = false;
        const saveRetireAsset = new SaveStornoAssetMFX();

        saveRetireAsset.refDocNo = this.refDocNo;
        saveRetireAsset.assetId = this.asset.id;

        if (saveRetireAsset != null) {
            this.assetHttpService.newStornoAssetMFX(saveRetireAsset)
