...
import { MonthDate } from '../../../common/event-emitter';
import { DialogService } from '../../../services/dialog.service';
import { AssetEditComponent } from './asset-edit-dialog/asset-edit.component';
import { MatDialog } from '@angular/material/dialog';
import { AssetSapOperationDialogComponent } from './asset-sap-operation-dialog/asset-sap-operation-dialog.component';
import { NewOperationDialogComponent } from './new-operation-dialog/new-operation-dialog.component';
import { AssetCloneDialogComponent } from './asset-clone-dialog/asset-clone-dialog.component';
import { AssetRetireDialogComponent } from './asset-retire-dialog/asset-retire-dialog.component';
...
import { AssetPublicRetireDialogComponent } from './asset-public-retire-dialog/asset-public-retire-dialog.component';
import {AssetStornoDialogComponent} from './asset-storno-dialog/asset-storno-dialog.component'
...
export class AssetManageComponent implements OnInit, AfterViewInit { // extends GenericManage<AssetInvDetail> {
    ...
    @Output() public showValuesEvent: EventEmitter<boolean> = new EventEmitter();
    @Output() public clearFilterEvent: EventEmitter<boolean> = new EventEmitter();
...
//   };
    ...
    constructor(public route: ActivatedRoute,
            ...
            this.router.events.subscribe((evt) => {
                if (evt instanceof NavigationEnd) {
                    if (evt.urlAfterRedirects === '/asset/inuse') {
    ...
    ngOnInit() {
      ...
      // this.date12.setFullYear(this.date12.getFullYear(), this.date12.getMonth() - 1);
    }
    ...
    public clearFilters() {
        ...
        this.clearFilterEvent.emit(this.resetFilters);
        // this.date12 = null;
        this.checkForRefresh();
        this.isPrinted = '-';
        this.isDuplicate = '-';
    ...
    // /* END DICTIONARY ITEM */
        ...
        public setSelectedUomNew() {
            const items: Array<Uom> = this.uomListNew.selectedItems;
        ...
        public setSelectedDimensionNew() {
            const items: Array<Dimension> = this.dimensionListNew.selectedItems;
    ...
    public editAsset() {
      ...
      if (selectedAssetId > 0) {
        // this.router.navigate(['/assetedit', selectedAssetId]);
        this.onEditItem(selectedAssetId);
      }
    }
    public editAssetOther(selectAssetId:number) {
        console.log('editAsset not called');
        const selectedAssetId = selectAssetId
        if (selectedAssetId > 0) {
          // this.router.navigate(['/assetedit', selectedAssetId]);
          this.onEditItem(selectedAssetId);
        }
    ...
    public addNewAssetFromStock() {
        this.router.navigate(['/newfromstock']);
    }

    public addNewOperation() {
        AppData.AssetList = this.selectedAssets;
        let isInTransfer = 0;
        const isWaitingValidation = 0;
        this.selectedAssets.forEach(asset => {
            if (asset.isInTransfer) {
                this.notifyService.showWarning('Numarul de inventar ' + asset.invNo + ' asteapta validarea transferului!', '', 3000, false, 3000);
    ...
    public refreshItems() {
        ...
        this.assetList.resetFilters(params);
        this.assetList.refresh(params);
      }

    public openNewOperationDialog() {
    ...
    public changeShowValues() {
    this.showValues = !this.showValues;
    this.showValuesEvent.emit(this.showValues);
    this.refreshAssets();
  }
    ...
    public transferAsset() {
        const selectedAssetId = this.selectedAssets.length > 0 ? this.selectedAssets[0].id : 0;
        if (selectedAssetId > 0) {
            // this.router.navigate(['/assetoperation', selectedAssetId]);
            this.openNewSAPOperationDialog(selectedAssetId);
        }
    }

    public openNewSAPOperationDialog(selectedAssetId) {
      const dialogRef = this.dialog.open(AssetSapOperationDialogComponent, {
        panelClass: 'large-modal', disableClose: true, width: '100%', maxWidth: '100vw', maxHeight: '100vh', height: 'calc(100vh - 55px)', position: { bottom: '0' },
    ...
    public cloneAsset() {
        const selectedAssetId = this.selectedAssets.length > 0 ? this.selectedAssets[0].id : 0;
        if (selectedAssetId > 0) {
            this.onCloneAssetDialog(selectedAssetId);
        }
    }

    public onCloneAssetDialog(selectedAssetId) {
      const dialogRef = this.dialog.open(AssetCloneDialogComponent, {
        panelClass: 'large-modal', disableClose: true, width: '100%', maxWidth: '100vw', maxHeight: '100vh', height: 'calc(100vh - 55px)', position: { bottom: '0' },
        data: { assetId: selectedAssetId }
      });
    ...
    public scrabAsset() {
        const selectedAssetId = this.selectedAssets.length > 0 ? this.selectedAssets[0].id : 0;
        if (selectedAssetId > 0) {
            // this.router.navigate(['/assetretire', selectedAssetId]);
            this.onScrabAssetDialog(selectedAssetId);
        }
    }

    public onScrabAssetDialog(selectedAssetId) {
      const dialogRef = this.dialog.open(AssetRetireDialogComponent, {
        panelClass: 'large-modal', disableClose: true, width: '100%', maxWidth: '100vw', maxHeight: '100vh', height: 'calc(100vh - 55px)', position: { bottom: '0' },
        data: { assetId: selectedAssetId }
      });
    ...
    public stornoAsset() {
        const selectedAssetId = this.selectedAssets.length > 0 ? this.selectedAssets[0].id : 0;
        if (selectedAssetId > 0) {
            // this.router.navigate(['/assetstorno', selectedAssetId]);
          this.onStornoAssetDialog(selectedAssetId);
        }
    }

  public onStornoAssetDialog(selectedAssetId) {
    const dialogRef = this.dialog.open(AssetRetireDialogComponent, {
      panelClass: 'large-modal', disableClose: true, width: '100%', maxWidth: '100vw', maxHeight: '100vh', height: 'calc(100vh - 55px)', position: { bottom: '0' },
      data: { assetId: selectedAssetId }
    });
    ...
    public stornoAcquisitionAsset() {
//         const selectedAssetId = this.selectedAssets.length > 0 ? this.selectedAssets[0].id : 0;
//         if (selectedAssetId > 0) {
//             this.router.navigate(['/assetacquisitionstorno', selectedAssetId]);
//         }

        const selectedAssetId = this.selectedAssets.length > 0 ? this.selectedAssets[0].id : 0;
        if (selectedAssetId > 0) {
            const dialogRef = this.dialog.open(AssetStornoDialogComponent, {
              panelClass: 'large-modal', disableClose: true, width: '100%', maxWidth: '100vw', maxHeight: '100vh', height: 'calc(100vh - 55px)', position: { bottom: '0' },
              data: { assetId: selectedAssetId }
      //         data: { assetId: 300793 }
            });
    ...
    public removeFromAssetCategorySelection(assetCategory: AssetCategory) {
        const index: number = this.selectedAssetCategories.indexOf(assetCategory);
    ...
    /*end asset category*/
        ...
        public removeFromInterCompanySelection(interCompany: InterCompany) {
            const index: number = this.selectedInterCompanies.indexOf(interCompany);
        ...
        /*end  INSURANCECATEGORY  */
            ...
            /*end ASSET NATURE */
                ...
                public removeFromDictionaryItemSelection(dictionaryItem: DictionaryItem) {
                    const index: number = this.selectedDictionaryItems.indexOf(dictionaryItem);
    ...
    public removeFromAssetClassSelection(assetClass: AssetClass) {
        const index: number = this.selectedAssetClasses.indexOf(assetClass);
    ...
    public removeFromBrandSelection(brand: Brand) {
        const index: number = this.selectedBrands.indexOf(brand);
   ...
   /* begin room */
    ...
    public deleteAsset() {
        ...
        public blockAsset() {
          this.dialogService
            ...
            .subscribe((confirmed: any) => {
              if (confirmed) {
                this.assetHttpService.blockAsset(this.selectedAssets[0].id).subscribe((res) => {
                  if (res) {
                    // this.newAssetModal.hide();
                    this.notifyService.showSuccess('Datele au fost actualizate cu succes!', '', 0, false, 0);
                    // this.toastr.success('Date au fost actualizate cu success');
                    ...
                    this.notifyService.showError('Eroare salvare!', '', 0, false, 0);
                  }
                }, (error) => {
                  this.notifyService.showError('Eroare server!', '', 0, false, 0);
                }, () => {
    ...
    public getFilters(): Array<Param> {
        ...
        assetFilter.isTemp = ((this.isTemp === '-') ? null : (this.isTemp === 'DA' ? true : false));
        ...
        assetFilter.monthYear = this.date12;
        assetFilter.showValues = this.showValues;
        assetFilter.export = 'inuse';
    ...
    public doImportThales() {
        if (this.importIndex < this.importLinesThales.length) {
            this.assetHttpService.uploadThales(this.importLinesThales[this.importIndex]).subscribe((data) => {
                this.importIndex = this.importIndex + 1;
                this.doImportThales();
            });
        } else {
            ...
            this.importDataModal.hide();
            this.importIndex = 0;
            this.importLinesThales = new Array<ImportThales>();
        }
    }
    ...
    public upload() {
        const fi = this.fileInputEmag.nativeElement;
    ...
    //         if (asset.isPrinted) {
    //             this.notifyService.showWarning('Numarul de inventar ' + asset.invNo + ' a fost printat deja!', '', 0, false, 0);
    //             this.refreshAssets();
    //             return;
    //         } else if (asset.name.length < 1) {
    //             this.notifyService.showWarning('Pentru numarul de inventar ' + asset.invNo + ' nu a fost selectata denumirea!', '', 0, false, 0);
    //             goToprint = false;
    //         } else {
...
//       this.mainViewMode = AssetManageMainViewMode.PrintLabel;
//       this.printMessage = "Doriti sa printati si eticheta finala?";
//       this.printModal.show();
// }

public onPrintCanceled() {
  this.mainViewMode = AssetManageMainViewMode.NotSet;
  this.printModal.hide();

}

public getJsonFilterEventEvent(event) {
  this.params = event;
  //console.log(event);
}

public getPropertyFilterEvent(event) {
   // this.params = event;
   //console.log(event)
   this.refreshAssets();
}
...
public generateMF() {
  const selectedAssetId = this.selectedAssets.length > 0 ? this.selectedAssets[0].id : 0;
  if (selectedAssetId > 0) {
      this.assetHttpService.generateMF(selectedAssetId).subscribe( (res)=> {
        fileSaveAs(res.body, 'Fisa mijloc fix.pdf');
      })
  }
}
...
openPrinter(zpl: string) {
  ...
  onMonthDateChanged($event: MonthDate) {
    this.date12 = $event.monthAndYear;
  ...
  public onStockTransfer() {
    ...
    this.selectedAssets.forEach(asset => {
        if (asset.isInTransfer) {
            this.notifyService.showWarning('Numarul de inventar ' + asset.invNo + ' asteapta validarea transferului!', '', 0, false, 0);
            isInTransfer++;
        }
    });
    ...
    if (isInTransfer > 0 || isWaitingValidation > 0) {
      ...
      this.viewMode = AssetManageViewMode.StockIT;
      // this.confirmationMessage = 'Doriti sa returnati catre IT echipamentul selectat?';
      // this.confirmationModal.show();

      this.dialogService
      .confirmDialog({
        title: 'Confirm Action',
        message: 'Doriti sa returnati catre IT echipamentul selectat?',
        confirmCaption: 'Da',
        cancelCaption: 'Nu',
      })
      .subscribe((confirmed) => {
  ...
  public stornoMF() {
    let isStorno = 0;
          this.selectedAssets.forEach(asset => {
              if (asset.storno) {
                  this.notifyService.showWarning('Numarul de inventar ' + asset.invNo + ' asteapta validarea stornarii!', '', 0, false, 0);
                  isStorno++;
              }
          });
          if (isStorno > 0) {
            ...
            this.viewMode = AssetManageViewMode.StornoMF;
            // this.confirmationMessage = 'Doriti sa stornati echipamentul selectat?';
            // this.confirmationModal.show();

            this.dialogService
            .confirmDialog({
              title: 'Confirm Action',
              message: 'Doriti sa stornati echipamentul selectat?',
              confirmCaption: 'Da',
              cancelCaption: 'Nu',
            })
            .subscribe((confirmed) => {
  ...
  if (isStorno > 0) {
    ...
    this.assetHttpService.stornoMF(ids).subscribe((data: RequestResult) => {
      if(data.success){
        this.isSaved = true;
        this.notifyService.showInfo(data.message, 'Stornare MF', 5000, false, 0);
        //alert('Operatia a fost validata cu success!');
        this.clearSelection();
        this.refreshAssets();
      } else {
        this.isSaved = true;
        this.notifyService.showError(data.message, 'Stornare MF', 0, false, 0);
      }
    });
  }
  }

  public scrabPublicAsset() {
     // this.openPublicRetireDialog(300786);
    const res = this.allowPublicCassation;
      if (!res) {
        this.notifyService.showWarning('Echipamentul selectat este intr un flux de casare', 'Propunere casare', 5000, false, 0);
        return;
      }
      const selectedAssetId = this.selectedAssets.length > 0 ? this.selectedAssets[0].id : 0;
      if (selectedAssetId > 0) {
          // this.router.navigate(['/assetpublicretire', selectedAssetId]);
          this.openPublicRetireDialog(selectedAssetId);
      }
  }

  public openPublicRetireDialog(selectedAssetId) {
    const dialogRef = this.dialog.open(AssetPublicRetireDialogComponent, {
      panelClass: 'large-modal', disableClose: true, width: '100%', maxWidth: '100vw', maxHeight: '100vh', height: 'calc(100vh - 55px)', position: { bottom: '0' },
      data: { assetId: selectedAssetId }
    });
  ...
  public saveDocument() {
    ...
    this.documentHttpService.saveFullDocument(documentUpload).toPromise().then((data) => {
      if(data.success){
        this.isSaved = true;
        this.notifyService.showInfo(data.message, 'Transfer catre IT', 2000, false, 0);
        //alert('Operatia a fost validata cu success!');
        this.clearSelection();
        this.refreshAssets();
      } else {
        this.isSaved = true;
        this.notifyService.showError(data.message, 'Transfer catre IT', 0, false, 0);
      }
  }, (error) => {
    alert('Eroare server');
  });
  ...
  public get allowPublicCassation(): boolean {
    if(this.selectedAssets.length > 0) {
     for (let index = 0; index < this.selectedAssets.length; index++) {
       const element = this.selectedAssets[index];
       if(element.cassation){
         return false;
       }
     }
     return true;
  ...
  public get allowPublicStorno(): boolean {
    if(this.selectedAssets.length > 0){
     for (let index = 0; index < this.selectedAssets.length; index++) {
       const element = this.selectedAssets[index];
       if(element.storno){
  ...
  public onConfirmationApproved() {
    ...
    switch (this.viewMode) {
        case AssetManageViewMode.StockIT:
            this.saveDocument();
          break;
          case AssetManageViewMode.StornoMF:
            this.onStornoMF();
          break;
        default:
            break;
    ...
    this.viewMode = AssetManageViewMode.NotSet;
