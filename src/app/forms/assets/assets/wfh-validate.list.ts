...
@Component({
    selector: 'wfh-validate-list',
    templateUrl: '../../assets/assets/wfh-validate.list.html',
    inputs: [ 'listTemplate' ],
    outputs: [ 'listAtferViewInit' ]
})
export class WFHValidateOfferListComponent extends GenericTableList<any, number> implements AfterViewInit {
    public query = '';
    public filteredList = [];
    public elementRef;
    public listTemplate: string = 'EMPLOYEENEWASSETS';
    ...
    public onAllowLabelInfoMinusUpdate(tableItem) {
      tableItem.item.infoMinus = tableItem.item.infoMinus;
      // tableItem.selected = !tableItem.selected;
      this.updateCheckStateEmp(true);
   }
...
onRateDateUpdate(event, tableItem) {
  // console.log(event.target.value);
