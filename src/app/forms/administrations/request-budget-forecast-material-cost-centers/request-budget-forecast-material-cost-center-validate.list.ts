...
@Component({
    selector: 'app-request-budget-forecast-material-cost-center-validate-list',
    templateUrl: './request-budget-forecast-material-cost-center-validate.list.html',
    inputs: [ 'listTemplate', 'materials' ],
    outputs: [ 'listAtferViewInit' ]
})
export class RequestBFMaterialCostCenterValidateListComponent extends GenericTableList<any, number> implements AfterViewInit {
    public query = '';
    public filteredList = [];
    public elementRef;
    public listTemplate: string = 'REQUESTBFMATERIALCOSTCENTERS';
      ...
      dialogRef.afterClosed().subscribe((materials: Material[]) => {
        ...
        if(materials.length > 0){
          materials.forEach(element => {
            concat = concat + element.code + ", ";
          });
          tableItem.item.materials = concat;
        }
      //   if (material) {
      //     // this.messageService.add({severity:'info', summary: 'Product Selected', detail: product.name});
      // }
