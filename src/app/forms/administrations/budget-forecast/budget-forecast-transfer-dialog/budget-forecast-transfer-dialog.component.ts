...
import {BudgetTransferSave} from '../../../../model/api/administration/budget-transfer-save';
...
@Component({
  ...
  templateUrl: './budget-forecast-transfer-dialog.component.html',
  styleUrls: ['./budget-forecast-transfer-dialog.component.scss']
})
export class BudgetForecastTransferDialogComponent implements AfterViewInit {
    ...
    public _destinationBudgetForecast: BudgetForecast = null;
    public get destinationBudgetForecast(): BudgetForecast { return this._destinationBudgetForecast; }
    public set destinationBudgetForecast(value: BudgetForecast) {
      this._destinationBudgetForecast = value;

      this.setSelectedBudgetForecast();
    }
   ...
   @ViewChild('masterTypeList') public masterTypeList: MasterTypeListComponent;
    ...
    public id: number = 0;
    public budget: BudgetTransferSave = new BudgetTransferSave();
    ...
    budgetSrcFinalValue: number = 0;
    budgetDstFinalValue: number = 0;

    budgetSrcFinalDynamicValue: number = 0;
    budgetDstFinalDynamicValue: number = 0;
    ...
    public get allowSaving(): boolean {
        ...
        (
          this.budgetDynamicValueCorrection > 0 && this.budgetValue > 0 &&
          (this.budgetDynamicValueCorrection - this.budgetValue === 0));
    }
    ...
    ngAfterViewInit() {
        if (this.id > 0) {
            this.budgetForecastHttpService.getDetailById(this.id)
                .subscribe((budgetForecast: any) => {
                  this.srcResult = budgetForecast;

                  this.updateDetails(budgetForecast);
...
// }
  ...
  /*end employee src*/
    ...
    public saveBudget() {
        ...
        this.budget.budgetForecastDestinationId = this.destinationBudgetForecast.id;
        ...
        if (this.budget.id > 0) {
             this.budgetForecastHttpService.transferBudgetForecast(this.budget)
            .subscribe((res: BudgetForecastCorrrectionResult) => {
              if(res.success){
                 //this.notificationService.showSuccess(res.message, 'Transfer buget', 2000, true, 0);
                 this.onAssetFileUpload(res.budgetBaseOpId);
                // this.budgetForecastHttpService.getDetailById(this.budget.id)
                // .subscribe((budgetForecast: any) => {
                //     if (budgetForecast != null) {
                //         this.isSaved = true;
                //         this.updateDetails(budgetForecast);
                //         this.onAssetFileUpload();
                //     }
                // }, (error) => {
    ...
    public onBudgetBaseOpDetailListSelectionChanged(assetOpDetails: Array<any>) {
    ...
    valueChangeApril(value) {
         ...
         this.budgetDynamicValue = Number(this.precise_round(((this.budget.aprilForecast + this.budget.mayForecast + this.budget.juneForecast + this.budget.julyForecast +
                                        ...
                                        this.budget.decemberForecast + this.budget.januaryForecast + this.budget.februaryForecast + this.budget.marchForecast) - this.budget.importValueOrder - this.budget.valueOrder), 2));
    ...
    valueChangeDecember(value) {
      ...
      this.budgetDynamicValue = Number(this.precise_round(((this.budget.aprilForecast + this.budget.mayForecast + this.budget.juneForecast + this.budget.julyForecast +
                                     this.budget.augustForecast + this.budget.septemberForecast + this.budget.octomberForecast + this.budget.novemberForecast +
                                     this.budget.decemberForecast + this.budget.januaryForecast + this.budget.februaryForecast + this.budget.marchForecast) - this.budget.importValueOrder - this.budget.valueOrder), 2));
    console.log(this.budgetDynamicValue);
     this.budgetValue = this.precise_round((this.budget.totalRem - this.budgetDynamicValue), 2);
    }
  ...
  valueChangeDestinationDecember(value) {
    ...
    this.budgetDynamicValueCorrection = Number(this.precise_round((this.budget.aprilForecastDstNew + this.budget.mayForecastDstNew + this.budget.juneForecastDstNew + this.budget.julyForecastDstNew +
      ...
      this.budget.decemberForecastDstNew + this.budget.januaryForecastDstNew + this.budget.februaryForecastDstNew + this.budget.marchForecastDstNew), 2));
       //this.budgetValueCorrection = this.precise_round((this.budget.totalRemDestination + this.budgetDynamicValueCorrection), 2);
       console.log(this.budgetDynamicValueCorrection);
       this.budgetValueCorrection = this.budget.totalRemDestination + this.budgetDynamicValueCorrection;
  }
  ...
  valueChangeDestinationMarch(value) {
    ...
    valueSrcChange(value) {
      this.budgetSrcFinalDynamicValue = 0;
      this.budgetSrcFinalDynamicValue = Number(this.precise_round((this.budget.totalRem - value), 2));
    }

    valueDstChange(value) {
      this.budgetDstFinalDynamicValue = 0;
      this.budgetDstFinalDynamicValue = Number(this.precise_round((this.budget.totalRemDestination + value), 2));
    }
...
public setSelectedBudgetForecast() {
  ...
  if(this.destinationBudgetForecast != null) {
    ...
    //   this.budget.decemberForecast + this.budget.januaryForecast + this.budget.februaryForecast + this.budget.marchForecast), 2));
      ...
      this.budgetSrcFinalValue = this.budget.totalRem;
      this.budgetDstFinalValue = this.destinationBudgetForecast?.totalRem;
...
onAssetFileUpload(budgetBaseOpId: number) {
  const dialogRef = this.dialog.open(UploadBudgetTransferModalComponent, {
    panelClass: 'centered-middle-modal', height: '80%', maxHeight: '80%', disableClose: true, width: '600px', position: { bottom: '15%', top: 'auto'},
    data: { uploadType: 'REQUEST', uploadFolder: 'REQUEST', assetId: this.id, budgetBaseOpId: budgetBaseOpId }
  });
