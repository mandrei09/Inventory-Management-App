...
import {OrderMaterialUpdate} from '../../../../model/api/administration/order-material-update';
...
import {ContractsSelectionDialog} from '../../contracts/selection/contracts.selection.dialog';
...
export class OrderDetailsDialogComponent implements OnInit, AfterViewInit {
    ...
    public set plantInitial(value: Plant) {
      this._plantInitial = value;

      this.setSelectedPlantInitial(value);
    }

  public _plantFinal: Plant = null;
  public get plantFinal(): Plant { return this._plantFinal; }
  public set plantFinal(value: Plant) {
   ...
   @ViewChild('masterTypeList') public masterTypeList: MasterTypeListComponent;
    ...
    public onContractAdd() {
      let dialogRef = this.dialog.open(ContractsSelectionDialog, { panelClass: 'centered-middle-modal', width: '80%', maxWidth: '90%', maxHeight: '80%', height: 'auto', position: {top: '10em'},
        data: { params: this.contractParams() }
      });

      dialogRef.afterClosed().subscribe((items: any) => {
        // this.requestBudgetForecastList.refresh();
        if(items.length > 0){
          this.setSelectedContract(items);
        }
    ...
    public onMaterialAdd() {
      let dialogRef = this.dialog.open(MaterialsSelectionDialog, { panelClass: 'centered-middle-modal', width: '80%', maxWidth: '90%', maxHeight: '80%', height: 'auto', position: {top: '10em'},
    ...
    public updateDetails(order: any) {
        ...
        /*end asset type*/
            ...
            // public get offerParams(): Array<Param> {
            ...
            public setSelectedOffer(value) {
                ...
                if (this.orderType != null && this.orderType.code != 'C-IT') {
                    ...
                    setTimeout(() => {
                      ...
                      this.requestBudgetForecastMaterialAllList.selectedItems = new Array<any>();
                    ...
                    setTimeout(() => {
                      ...
                      // console.log( this.offerMaterialList.items);
          ...
          // /* end BUDGET FORECAST */
              ...
              public setSelectedContract(value: any) {

                  this.contract = value[0];
  ...
  //   this.requestBudgetForecastList.selectedItem != null ? this.requestBudgetForecastList.selectedItem : null ])));
    ...
    public saveOrder() {
      ...
      this.order.multiple = new Array<boolean>();
      ...
      this.requestBudgetForecastMaterialList.items.forEach(element => {
        this.order.multiple.push(element.multiple);
      });
      this.isSaved = false;
      ...
      this.blocked = true;
      this.orderHttpService.addNewOrder(this.order).subscribe((result: OrderResult) => {
        if (result.success) {
          this.notificationService.showInfo(result.message, 'P.O. nou', 5000, false, 0);
          //this.router.navigate(['/procurement/order']);
    ...
    public saveOrderStock() {
      ...
      if (this.orderType != null && this.orderType.code === 'C-IT') {
            ...
            .subscribe((result: OrderResult) => {
                if (result.success) {
                  this.notificationService.showInfo(result.message, 'Comanda noua', 5000, false, 0);
                  //this.router.navigate(['/procurement/order']);
                  this.orderCheck = false;
                  this.blocked = false;
...
private selectedMaterials(): Array<CodeNameEntity> {
    ...
    if (this.offerITMaterialList && this.offerITMaterialList.items.length > 0) {
...
public setSelectedMaterial(value) {
  ...
  this.requestBudgetForecastMaterialHttpService.addMaterialByRequestBudgetForecast(materialsIds).subscribe( (res) => {
          if (res.statusCode === 200) {
              this.notificationService.showInfo('Datele au fost salvate cu success!', 'Adauga mapare material', 5000, false, 0);
              this.materialList.refresh(null);
              ...
              this.requestBudgetForecastList.refresh(null);
              this.materialList.selectedItems = new Array<Material>();
              console.log(this.materialList.selectedItems)
          } else if (res.statusCode === 404) {
              this.notificationService.showError('Nu exista', 'Adauga materiale', 0, false, 0);
              this.materialList.selectedItems = new Array<Material>();
          }
...
public get canYouAddValue(): boolean {
  if(this.contract)
  {
    const sum = this.contract.contractAmount.amountRem;
    const materialsSum = this.offerMaterialList.items.reduce((sum, current) => sum + current.priceIni, 0);
    if(sum)
      if(materialsSum > sum)
        return false
    return true
...
public get sumMaxMaterials(): boolean {
  ...
  /* END SUBCATEGORY  */
    ...
    public setSelectedPlantInitial(value) {
        this.stockList.refresh(null);
        // const items: Array<Plant> = this.plantInitialList.selectedItems;
        const items: Array<Plant> = value;
        // this.plantInitial = value;
...
// }
 ...
 public selectAssetEntity() {
   let dialogRef = this.dialog.open(AssetEntitySimpleSelectionDialog, { panelClass: 'centered-middle-modal', width: '80%', maxWidth: '90%', maxHeight: '80%', height: 'auto', position: {top: '10em'},
     data: {  }
   });

   dialogRef.afterClosed().subscribe((items: any) => {
     if (items.length > 0) {
       this.setSelectedAssetEntity(items[0]);
     }
