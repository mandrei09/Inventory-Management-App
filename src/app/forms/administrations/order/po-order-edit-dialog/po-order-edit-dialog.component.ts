...
import { BudgetForecastSelectionDialog } from '../../budget-forecast/selection/budget-forecast.selection.dialog';
import { PoOrderUploadModalComponent } from '../../../common/upload-modal/po-order.upload-modal.component';

@Component({
  selector: "app-po-order-edit-dialog",
  templateUrl: "./po-order-edit-dialog.component.html",
  styleUrls: ["./po-order-edit-dialog.component.scss"],
})
export class PoOrderEditDialogComponent implements OnInit, AfterViewInit {
  ...
  @ViewChild("projectTypeDivisionList")
  public projectTypeDivisionList: ProjectTypeDivisionListComponent;
  @ViewChild("projectTypeDivisionListModal")
  public projectTypeDivisionListModal: ModalDirective;
  ...
  @ViewChild("budgetForecastList")
  public budgetForecastList: BudgetForecastListComponent;
  @ViewChild("budgetForecastListModal")
  public budgetForecastListModal: ModalDirective;
  ...
  @ViewChild("startAccMonthList")
  public startAccMonthList: AccMonthListComponent;
  @ViewChild("startAccMonthListModal")
  public startAccMonthListModal: ModalDirective;

  @ViewChild("requestBudgetForecastList")
  public requestBudgetForecastList: RequestBudgetForecastListComponent;
  @ViewChild("requestBudgetForecastListModal")
  public requestBudgetForecastListModal: ModalDirective;
  ...
  @ViewChild("assetReceptionList")
  public assetReceptionList: AssetListComponent;
  ...
  public selectedprojectTypeDivisions: Array<ProjectTypeDivision> =
    new Array<ProjectTypeDivision>();
  public selectedRequestBudgetForecasts: Array<AssetSimpleDetail> =
  ...
  public get selectedOwner(): EmployeeResource {
  ...
  public get selectedAppState(): CodeNameEntity {
    return this._selectedAppState;
  }
  ...
  public get selectedDivision(): CodeNameEntity {
  ...
  public get selectedAssetType(): CodeNameEntity {
    return this._selectedAssetType;
  }
  ...
  public get allowSaving(): boolean {
    return (
      this.order != null &&
      this.order.info != null &&
      this.order.info.trim().length > 3 &&
      this.employee != null &&
  ...
  public onAddPreReception() {
    let dialogRef = this.dialog.open(AssetAddDialogComponent, {
      ...
      position: { bottom: "0" },
      data: { order: this.order, isPreReception: true },
    });

    dialogRef.afterClosed().subscribe((item: CreateAssetSAPResult) => {
      if (item.success) this.refreshReceptionList();
    });
  }
  ...
  public selectType() {
    ...
    params.push(
      new Param(
        "masterTypeIds",
        this.masterType != null ? this.masterType.id.toString() : null
      )
    );
  ...
  public selectSubType() {
    ...
    params.push(
      new Param("typeIds", this.type != null ? this.type.id.toString() : null)
    );
  ...
  public get addInvoiceNo() {
    return this.assetReceptionList?.tableItems?.every(tableItem => 
      tableItem?.item?.docNo1 !== null && 
      tableItem?.item?.docNo1 !== undefined && 
      tableItem?.item?.docNo1 !== ""
    ) ?? false;
  }
  ...
  public selectInterCompany() {
    ...
    params.push(
      new Param(
        "partnerIds",
        this.partner != null ? this.partner.id.toString() : null
      )
    );
  ...
  public setSelectedAssetType() {
    ...
    if (
      this.projectTypeDivision != null &&
      this.projectTypeDivision.projectType != null
    ) {
      budgetFilter.projectTypeIds = new Array<number>();
      budgetFilter.projectTypeIds.push(this.projectTypeDivision.projectType.id);
    }
    ...
    if (
      this.projectTypeDivision != null &&
      this.projectTypeDivision.division != null
    ) {
      budgetFilter.divisionIds = new Array<number>();
      budgetFilter.divisionIds.push(this.projectTypeDivision.division.id);
    }
    ...
    this.budgetForecastHttpService
      .get(1, 500, "id", "asc", params, null, "detailui")
      .subscribe((res: any) => {
  ...
  public setSelectedProjectTypeDivision() {
    const items: Array<ProjectTypeDivision> =
      this.projectTypeDivisionList.selectedItems;
    this.projectTypeDivision =
  ...
  onAssetPreReceptionFileUpload() {
    const assetIds: number[] = this.assetReceptionList.selectedItems.map(item => item.id);
    const dialogRef = this.dialog.open(UploadPreReceptionModalComponent, {
      ...
      data: {
        uploadType: "PRE_RECEPTION",
        uploadFolder: "PRE_RECEPTION",
        assetIds: assetIds,
        docNo1: this.assetReceptionList.selectedItems[0].docNo1,
        details: this.assetReceptionList.selectedItems[0].details,
      },
    });
  ...
  public processAssetOp() {
    this.requestOpHttpService
      .process(this.requestOpList.selectedItem.id)
      .subscribe((data) => {
  ...
  onProjectTypeDivisionListFocus() {
    if (this.projectTypeDivisions.length === 0) {
      this.refreshProjectTypeDivisionList();
    }
  }
  ...
  refreshProjectTypeDivisionList(): Observable<boolean> {
    ...
    this.requestHttpService.getData(params).subscribe(
      (res: any[]) => {
      ...
      () => {
        this.isProjectTypeDivisionLoading = false;
      }
    );
  ...
  public get allowNeedBudget(): boolean {
    return (
      ...
      this.assetType != null &&
      this.order != null &&
      this.order.budgetValueNeed > 100
    );
  }
  ...
  needBudget() {
    ...
    if (
      confirm(
        "ATENTIE! NU UITA SA INCARCI DOVADA DOCUMENTULUI PENTRU BUGETUL NOU?"
      )
    ) {
      const needBgt = new NeedBudget();
      needBgt.projectTypeDivisionId =
        this.projectTypeDivision != null ? this.projectTypeDivision.id : null;
      ...
      needBgt.startAccMonthId =
        this.startAccMonth != null ? this.startAccMonth.id : null;
        ...
        .subscribe((result: RequestResult) => {
          if (result.success) {
            this.notificationService.showSuccess(
              result.message,
              "Purchase Request(PR) -solicitare buget",
              0,
              false,
              0
            );
  ...
  refreshAppStateList(): Observable<boolean> {
    ...
    this.appStateHttpService.getDetailByParentCode("REQUEST").subscribe(
      (res: any[]) => {
        this.appStates = res;
        resultSubject.next(true);
      },
  ...
  refreshOwnerList(): Observable<boolean> {
    ...
    this.employeeHttpService.get(1, 10000, "email", "asc", params).subscribe(
      (res: any) => {
  ...
  refreshDivisionList(): Observable<boolean> {
    ...
    this.divisionHttpService.getData(params).subscribe(
      (res: any[]) => {
        this.divisions = res;
        resultSubject.next(true);
      },
  ...
  refreshProjectTypeList(): Observable<boolean> {
    ...
    this.projectTypeHttpService.get(1, 1000, "name", "asc", params).subscribe(
      (res: any) => {
      ...
      () => {
        this.isProjectTypeLoading = false;
      }
    );
  ...
  refreshAssetTypeList(): Observable<boolean> {
    ...
    this.assetTypeHttpService.get(1, 1000, "name", "asc", params).subscribe(
      (res: any) => {
  ...
  update() {
    ...
    this.orderHttpService.editOrder(data).subscribe((res: RequestResult) => {
      if (res.success) {
        ...
        this.orderHttpService.getDetailById(this.id).subscribe((order: any) => {
          this.updateDetails(order);
        });
      }
    });
...
updateOrderBudgetForecast() {
  ...
  let dialogRef = this.dialog.open(BudgetForecastSelectionDialog, { panelClass: 'centered-middle-modal', width: '80%', maxWidth: '90%', maxHeight: '80%', height: 'auto', position: {top: '10em'}});
    ...
    dialogRef.afterClosed().subscribe((item: any) => {
      ...
      data.budgetForecastId = item[0].id as number;
      ...
      this.orderHttpService.updateOrderBudgetForecast(data).subscribe((res: RequestResult)=> {
        if(res.success) {
          ...
          this.notificationService.showError(res.message, '..', 2000, true, 0);
        }
      })
    });
}
  ...
  public setSelectedItem(reqBF: Array<any>) {
    ...
    this.requestBudgetForecastList.selectedItems.forEach((requestBGF) => {
      ...
      this.budgetForecastNew = requestBGF.budgetForecast
    });
    //   this.refreshEntityFiles();
    // }
  }
  ...
  public refreshBudgetForecastList(order: OrderEdit) {
    ...
    params.push(
      new Param(
        "requestIds",
        order != null && order.offer != null && order.offer.request != null
          ? order.offer.request.id.toString()
          : null
      )
    );
  ...
  public refreshOfferMaterialList() {
    ...
    params.push(
      new Param(
        "requestIds",
        this.order != null ? this.order.offer.request.id.toString() : null
      )
    );
    ...
    if (index == 5) {
      if (this.order != null) {
        this.matrixHttpService
          .matchmatrixeditpanel(this.order.id)
          .subscribe((res: MatrixTree[]) => {
            // console.log(res);
            this.data2 = [...res];
          });
      }
  ...
  public setSelectedBudgetForecast() {
    const items: Array<BudgetForecast> = this.budgetForecastList.selectedItems;
    this.budgetForecastNew =
      items != null && items.length === 1 ? items[0] : null;

    this.budgetForecastListModal.hide();
}

public refreshEntityFile(entityFiles: EntityFile[], entityFileId: number) {
...
onPoDocumentUpload(entityFiles: EntityFile[], entityFileId: number) {
  const dialogRef = this.dialog.open(PoOrderUploadModalComponent, {
    panelClass: 'centered-middle-modal', height: '80%', maxHeight: '80%', disableClose: true, width: '600px', position: { bottom: '15%', top: 'auto'},
    data: { orderId: this.order.id, uploadFolder: 'REQUESTBUDGETFORECAST'}
  });
  dialogRef.afterClosed().subscribe(() => {
    this.refreshEntityFiles();
  });
