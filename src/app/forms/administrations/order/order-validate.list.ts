...
import { AssetDepPagedResult } from "../../../model/api/assets/asset-dep-paged-result";

@Component({
  selector: "order-validate-list",
  templateUrl: "order-validate.list.html",

  inputs: ["listTemplate"],
  outputs: ["listAtferViewInit"],
})
export class OrderValidateList
  ...
  @Output()
  public listAtferViewInit: EventEmitter<void> = new EventEmitter<null>();
  ...
  public updateCheckStateEmp(checked: boolean) {
    if (checked) this.selectAllEmp();
    else this.unselectAllEmp();
  }
