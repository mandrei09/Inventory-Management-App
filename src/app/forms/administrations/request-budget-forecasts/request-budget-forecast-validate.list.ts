...
@Component({
    selector: 'app-request-budget-forecast-validate-list',
    templateUrl: './request-budget-forecast-validate.list.html',
    styleUrls: ['request-budget-forecast-validate.list.scss'],
    inputs: [ 'listTemplate', 'materials' ],
    outputs: [ 'listAfterViewInit', 'mappingFinished' ]
})
export class RequestBudgetForecastValidateOfferListComponent extends GenericTableList<any, number> implements AfterViewInit {
  ...
  paginate(event) {
    ...
    public onAllowMultipleUpdate(i) {
        this.tableItems[i].item.multiple = !!this.tableItems[i].item.multiple
         // tableItem.selected = !tableItem.selected;
        this.updateCheckStateEmp(true);
   }
