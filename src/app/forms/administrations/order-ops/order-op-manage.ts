...
import { OrderOpDetailList } from './order-op.detail.list';
...
@Component({
    selector: 'order-op-manage',
    templateUrl: 'order-op-manage.html',
    styleUrls: ['order-op-manage.scss'],
    providers: [
...
//   };
    ...
    public getFilters(): Array<Param> {
        ...
        let assetFilter: OrderFilter = new OrderFilter();
    ...
    public processAssetOp() {
        this.orderOpList.selectedItems.forEach( item => {
            if (this.selectedAssetOpIds.indexOf(item)  === -1){
                this.selectedAssetOpIds.push(item.id);
            }
        });

        this.orderOpHttpService.process(this.selectedAssetOpIds).subscribe((data) => {
            this.refreshAssetOperations();
        });
    }
    // public sendEmail() {
    ...
    public clearFilters() {
      ...
      //  this.assetOpState = '-';
        ...
        this.orderOpList.selectedItems = [];
        this.checkForRefresh();
    }
   ...
   /* begin room */
    ...
    public deleteOperation(){
        if(confirm('Esti sigur ca vrei sa anulezi transferul?')){
            this.orderOpHttpService.deleteAssetOp(this.selectedAssetOp.id).subscribe((res) => {});
            this.checkForRefresh();
        }
}
...
public validateOperationTemp(){
    ...
    if(this.selectedAssetOp.documentType.id != 11){
        ...
        if(confirm('Are you sure you want to validate the temporary transfer?')){
            this.orderOpHttpService.validateAssetOpTemp(this.selectedAssetOp.id).subscribe(() => {});
            this.checkForRefresh();
        }
    }
}
    ...
    public assetOpListExport() {
        ...
        this.orderOpHttpService.get(1, 1000000, 'modifiedAt', 'desc', params, null, 'details').subscribe(
            (result: PagedResult<any>) => {
                ...
                result.items.forEach((item: any) => {
                    ...
                    this.orderOpHttpService.exportAssetOps(item.id).subscribe((data) => {
                    });
                });
        ...
        this.orderOpHttpService.get(1, 1000000, 'modifiedAt', 'desc', params, null, 'details').subscribe(
            (result: PagedResult<any>) => {
              ...
              //  this.assetOpHttpService.downloadMailOps(items).subscribe((data) => {  // PIRAEUS
                this.orderOpHttpService.downloadMailOpsBnr(items).subscribe((data) => {  // BNR
                    });
                  //  setTimeout(5000);
                    this.orderOpHttpService
                  //  .download()  PIRAEUS
                    .downloadBnr() // BNR
                    .subscribe((blob) => {
        ...
        this.orderOpHttpService.get(1, 1000000, 'modifiedAt', 'desc', params, null, 'details').subscribe(
            (result: PagedResult<any>) => {
               ...
               //     this.assetOpHttpService.sendEmail(items).subscribe((data) => {  // PIRAEUS
                    ...
                    this.orderOpHttpService.sendEmailPiraeus(items).subscribe((data) => {  // PIRAEUS
    ...
    public doImport() {
        ...
        if (this.importIndex < this.importLines.length) {
            ...
            this.orderOpHttpService.upload(this.importLines[this.importIndex]).subscribe((data) => {
