...
import { OfferOpDetailList } from './offer-op.detail.list';
...
@Component({
    selector: 'offer-op-manage',
    templateUrl: 'offer-op-manage.html',
    styleUrls: ['offer-op-manage.scss'],
    providers: [
...
//   };
    ...
    public processAssetOp() {
        this.offerOpList.selectedItems.forEach( item => {
            if (this.selectedAssetOpIds.indexOf(item)  === -1){
                this.selectedAssetOpIds.push(item.id);
            }
        });

        this.offerOpHttpService.process(this.selectedAssetOpIds).subscribe((data) => {
            this.refreshAssetOperations();
        });
    }
    // public sendEmail() {
    ...
    public clearFilters() {
      ...
      //  this.assetOpState = '-';
        ...
        this.offerOpList.selectedItems = [];
        this.checkForRefresh();
    }
   ...
   /* begin room */
    ...
    public deleteOperation(){
        if(confirm('Esti sigur ca vrei sa anulezi transferul?')){
            this.offerOpHttpService.deleteAssetOp(this.selectedAssetOp.id).subscribe((res) => {});
            this.checkForRefresh();
        }
}
...
public validateOperationTemp(){
    ...
    if(this.selectedAssetOp.documentType.id != 11){
        ...
        if(confirm('Are you sure you want to validate the temporary transfer?')){
            this.offerOpHttpService.validateAssetOpTemp(this.selectedAssetOp.id).subscribe(() => {});
            this.checkForRefresh();
        }
    }
}
    ...
    public assetOpListExport() {
        ...
        this.offerOpHttpService.get(1, 1000000, 'modifiedAt', 'desc', params, null, 'details').subscribe(
            (result: PagedResult<any>) => {
                ...
                result.items.forEach((item: any) => {
                    ...
                    this.offerOpHttpService.exportAssetOps(item.id).subscribe((data) => {
                    });
                });
        ...
        this.offerOpHttpService.get(1, 1000000, 'modifiedAt', 'desc', params, null, 'details').subscribe(
            (result: PagedResult<any>) => {
              ...
              //  this.assetOpHttpService.downloadMailOps(items).subscribe((data) => {  // PIRAEUS
                this.offerOpHttpService.downloadMailOpsBnr(items).subscribe((data) => {  // BNR
                    });
                  //  setTimeout(5000);
                    this.offerOpHttpService
                  //  .download()  PIRAEUS
                    .downloadBnr() // BNR
                    .subscribe((blob) => {
        ...
        this.offerOpHttpService.get(1, 1000000, 'modifiedAt', 'desc', params, null, 'details').subscribe(
            (result: PagedResult<any>) => {
               ...
               //     this.assetOpHttpService.sendEmail(items).subscribe((data) => {  // PIRAEUS
                    ...
                    this.offerOpHttpService.sendEmailPiraeus(items).subscribe((data) => {  // PIRAEUS
    ...
    public doImport() {
        ...
        if (this.importIndex < this.importLines.length) {
            ...
            this.offerOpHttpService.upload(this.importLines[this.importIndex]).subscribe((data) => {
