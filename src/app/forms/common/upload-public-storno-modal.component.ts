...
@Component({
  selector: 'app-upload-public-storno-modal',
  templateUrl: './upload-public-storno-modal.component.html',
  styleUrls: ['./upload-public-storno-modal.component.scss']
})
export class UploadPublicStornoModalComponent implements OnInit {
  ...
  constructor (
    ...
    public dialogRef: MatDialogRef<UploadPublicStornoModalComponent>,
  ...
  public uploadFiles(index: number) {
    if (this.files.length > index) {
      ...
      this.entityFilesProvider.uploadPublicStorno({ fileName: file.name, data: file }, this.assetId, this.entityTypeId, this.info)
        .subscribe(() => {
