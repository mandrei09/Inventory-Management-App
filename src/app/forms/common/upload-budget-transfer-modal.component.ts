...
@Component({
  selector: 'app-upload-budget-transfer-modal',
  templateUrl: './upload-budget-transfer-modal.component.html',
  styleUrls: ['./upload-budget-transfer-modal.component.scss']
})
export class UploadBudgetTransferModalComponent implements OnInit {
  ...
  public budgetBaseOpId: number = 0;
  ...
  constructor (
    ...
    public dialogRef: MatDialogRef<UploadBudgetTransferModalComponent>,
    public notificationService: NotificationService,
    public notifyService: NotifyService,
    @Inject(MAT_DIALOG_DATA) public data: any
  ) {
    // console.log(JSON.stringify(data.item));
    //this.requestId =  data.item.requestId;
  ...
  private updateInputData(data: any) {
    ...
    this.budgetBaseOpId = data.budgetBaseOpId;
  }
  ...
  public uploadFiles(index: number) {
    ...
    if (this.files.length > index) {
      ...
      this.entityFilesProvider.uploadRequestTransferBudgetForecastDocument({ fileName: file.name, data: file }, this.assetId, this.budgetBaseOpId, this.entityTypeId, this.info, this.quantity, count)
