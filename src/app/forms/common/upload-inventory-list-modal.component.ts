...
@Component({
  selector: 'app-upload-inventory-list-modal',
  templateUrl: './upload-inventory-list-modal.component.html',
  styleUrls: ['./upload-inventory-list-modal.component.scss']
})
export class UploadInventoryListModalComponent implements OnInit {
  ...
  public isUploadAllowed : boolean = false

  constructor (
    public entityFilesProvider: EntityFileHttpService,
    public documentTypesProvider: EntityTypeHttpService,
    public dialogRef: MatDialogRef<UploadInventoryListModalComponent>,
  ...
  public uploadFiles(index: number) {
    if (this.files.length > index) {
      ...
      this.entityFilesProvider.uploadInventoryList({ fileName: file.name, data: file }, this.costCenterId, this.inventory.id, this.entityTypeId, this.info)
        .subscribe((res) => 
        {
        ...
        (error) => {  
          if(error.status==418)
            {
              this.notification.showWarning('Documentul se afla deja in baza de date!', '', 0, false, 0)
              return;
            }   
          else
            if(error.status==419)
            {
              this.notification.showWarning('Incarcare nereusita! Document generat pe alt centru de cost!', '', 0, false, 0)
              return;
            }                       
        }
        );
