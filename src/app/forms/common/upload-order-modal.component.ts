...
@Component({
  selector: 'app-upload-order-modal',
  templateUrl: './upload-order-modal.component.html',
  styleUrls: ['./upload-order-modal.component.scss']
})
export class UploadOrderModalComponent implements OnInit {
  ...
  constructor (
    ...
    public dialogRef: MatDialogRef<UploadOrderModalComponent>,
  ...
  public uploadFiles(index: number) {
    ...
    if (this.files.length > index) {
      ...
      this.entityFilesProvider.uploadOrder({ fileName: file.name, data: file }, this.assetId, this.reqBFId, this.entityTypeId, this.info, count)
