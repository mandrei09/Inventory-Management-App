...
@Component({
    selector: 'app-email-status-order-validate-L1-page',
    templateUrl: 'email-status-order-validate-L1-page.html'
})
export class EmailStatusOrderValidateL1PageComponent implements OnInit {
  position: string;
  reason: string = '';
  guid: string = '';
  ...
  confirmPosition(position: string) {
      ...
      this.dialogService
        ...
        .subscribe((confirmed: any) => {
          if (confirmed) {
            this.orderHttpService.validateMobileLevel1(this.guid).subscribe( (res: CreateAssetSAPResult)=> {
              if(res.success){
            ...
            switch(type) {
              case ConfirmEventType.REJECT:
                this.orderHttpService.rejectMobileLevel1(this.guid).subscribe( (res: CreateAssetSAPResult) => {
                  if(res.success){
