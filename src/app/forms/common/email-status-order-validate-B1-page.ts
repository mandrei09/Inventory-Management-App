...
@Component({
    selector: 'app-email-status-order-validate-B1-page',
    templateUrl: 'email-status-order-validate-B1-page.html'
})
export class EmailStatusOrderValidateB1PageComponent implements OnInit {
    position: string;
    reason: string = '';
    guid: string = '';
    ...
    confirmPosition(position: string) {
      ...
      this.dialogService
        ...
        .subscribe((confirmed: any) => {
          if (confirmed) {
            this.orderHttpService.validateMobileLevelB1(this.guid).subscribe( (res: CreateAssetSAPResult) => {
              if (res.success) {
                this.messageService.add({severity: 'info', summary: 'Aprobare', detail: 'Comanda a fost initiata cu sucess!'});
              } else {
            ...
            switch (type) {
              case ConfirmEventType.REJECT:
                this.orderHttpService.rejectMobileLevelB1(this.guid).subscribe( (res: CreateAssetSAPResult) => {
                  if (res.success) {
