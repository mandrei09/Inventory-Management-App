...
@Component({
    selector: 'app-email-status-order-validate-L4-page',
    templateUrl: 'email-status-order-validate-L4-page.html'
})
export class EmailStatusOrderValidateL4PageComponent implements OnInit {
  position: string;
  reason: string = '';
  guid: string = '';
  ...
  confirmPosition(position: string) {
      ...
      this.dialogService
        ...
        .subscribe((confirmed: any) => {
          if (confirmed) {
            this.orderHttpService.validateMobileLevel4(this.guid).subscribe( (res: CreateAssetSAPResult)=> {
              if (res.success) {
            ...
            switch(type) {
              case ConfirmEventType.REJECT:
                this.orderHttpService.rejectMobileLevel4(this.guid).subscribe( (res: CreateAssetSAPResult) => {
                  if(res.success){
