...
@Component({
    selector: 'app-email-status-order-validate-L3-page',
    templateUrl: 'email-status-order-validate-L3-page.html'
})
export class EmailStatusOrderValidateL3PageComponent implements OnInit {
  position: string;
  reason: string = '';
  guid: string = '';
  ...
  confirmPosition(position: string) {
      ...
      this.dialogService
        ...
        .subscribe((confirmed: any) => {
          if (confirmed) {
            this.orderHttpService.validateMobileLevel3(this.guid).subscribe( (res: CreateAssetSAPResult)=> {
              if (res.success) {
            ...
            switch(type) {
              case ConfirmEventType.REJECT:
                this.orderHttpService.rejectMobileLevel3(this.guid).subscribe( (res: CreateAssetSAPResult) => {
                  if(res.success){
