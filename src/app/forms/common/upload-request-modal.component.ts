...
export class UploadRequestModalComponent implements OnInit {
  ...
  constructor (
    ...
    public dialogRef: MatDialogRef<UploadRequestModalComponent>,
  ...
  public uploadFiles(index: number) {
    ...
    if (this.files.length > index) {
      ...
      if(this.uploadType === 'REQUEST_DOCUMENT'){
        this.entityFilesProvider.uploadRequestDocument({ fileName: file.name, data: file }, this.assetId, this.entityTypeId, this.info, this.quantity, count)
        ...
        this.entityFilesProvider.uploadRequest({ fileName: file.name, data: file }, this.assetId, this.entityTypeId, this.info, this.quantity, count)
        .subscribe((result: EntityFileResult) => {
          if (result.success) {
            this.notificationService.showInfo(result.message, 'Upload fisier', 5000, false, 0);
            this.uploadFiles(index + 1);
