...
@Component({
    selector: 'app-email-status-order-validate-S3-page',
    templateUrl: 'email-status-order-validate-S3-page.html'
})
export class EmailStatusOrderValidateS3PageComponent implements OnInit {
  position: string;
  reason: string = '';
  guid: string = '';
  ...
  confirmPosition(position: string) {
      ...
      this.dialogService
        ...
        .subscribe((confirmed: any) => {
          if (confirmed) {
            this.orderHttpService.validateMobileLevelS3(this.guid).subscribe( (res: CreateAssetSAPResult)=> {
              if(res.success){
            ...
            switch(type) {
              case ConfirmEventType.REJECT:
                this.orderHttpService.rejectMobileLevelS3(this.guid).subscribe( (res: CreateAssetSAPResult) => {
                  if(res.success){
