...
@Component({
    selector: 'app-email-status-order-validate-S1-page',
    templateUrl: 'email-status-order-validate-S1-page.html'
})
export class EmailStatusOrderValidateS1PageComponent implements OnInit {
  position: string;
  reason: string = '';
  guid: string = '';
  ...
  confirmPosition(position: string) {
    ...
    this.dialogService
      ...
      .subscribe((confirmed: any) => {
        if (confirmed) {
          this.orderHttpService.validateMobileLevelS1(this.guid).subscribe( (res: CreateAssetSAPResult)=> {
            if(res.success){
          ...
          switch(type) {
            case ConfirmEventType.REJECT:
              this.orderHttpService.rejectMobileLevelS1(this.guid).subscribe( (res: CreateAssetSAPResult) => {
                if(res.success) {
